...
class GameEngine:
    def start_game(self, request: KakaoWebhookRequest) -> GameResult:
    ...
    def load_status(self, request: KakaoWebhookRequest) -> GameResult:
    ...
    def advance_turn(self, request: KakaoWebhookRequest) -> GameResult:
        save = self._get_or_create(request.user.kakao_user_key)
    ...
    def build_menu(self, request: KakaoWebhookRequest) -> GameResult:
        save = self._get_or_create(request.user.kakao_user_key)
        options = [
            {
    ...
    def build(self, request: KakaoWebhookRequest) -> GameResult:
        save = self._get_or_create(request.user.kakao_user_key)
    ...
    def department_menu(self, request: KakaoWebhookRequest) -> GameResult:
        save = self._get_or_create(request.user.kakao_user_key)
        options = []
    ...
    def department(self, request: KakaoWebhookRequest) -> GameResult:
        save = self._get_or_create(request.user.kakao_user_key)
    ...
    def admission_menu(self, request: KakaoWebhookRequest) -> GameResult:
        save = self._get_or_create(request.user.kakao_user_key)
        return GameResult(
            message=(
    ...
    def logs(self, request: KakaoWebhookRequest) -> GameResult:
        save = self._get_or_create(request.user.kakao_user_key)
        return GameResult(
            message="최근 운영 기록입니다.",
            logs=save.logs[:5],
    ...
    def _get_or_create(self, user_key: str) -> SaveState:
        save = save_repository.get(user_key)
        if save is None:
            save = self._initial_save(user_key)
            save_repository.put(user_key, save)
        return save
