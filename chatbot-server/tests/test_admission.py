...
def test_admission_fires_in_march(engine: GameEngine, repo: InMemorySaveRepository, user_key: str) -> None:
    """Advancing from month 2 → 3 should trigger admission and include '신입생' in logs."""
    repo.put(user_key, make_save(user_key, month=2, enrolled=50))
...
def test_policy_change(engine: GameEngine, repo: InMemorySaveRepository, user_key: str) -> None:
    """Calling admission() with ACTION_ADMISSION_HARD should update the policy to 'hard'."""
    repo.put(user_key, make_save(user_key, admission_policy="normal"))
    webhook = make_webhook(user_key, action_name="ACTION_ADMISSION_HARD")

    result = engine.admission(webhook)
